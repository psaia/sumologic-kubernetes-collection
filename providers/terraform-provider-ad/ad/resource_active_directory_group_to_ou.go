...
func resourceGroupToOU() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"group_manager": {
				Type:        schema.TypeString,
				Description: "The DN of the AD object to set as the manager of the group.",
				Optional:    true,
			},
			"gid_number": {
				Type:        schema.TypeString,
...
func resourceADGroupToOUCreate(d *schema.ResourceData, meta interface{}) error {
	...
	groupManager := d.Get("group_manager").(string)
	...
	err := addGroupToAD(groupName, dnOfGroup, client, description, gidNumber, groupManager)
	if err != nil {
...
func resourceADGroupToOUUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Name of the DN is : %s ", dnOfGroup)
	log.Printf("[DEBUG] Modifying the Group: %s ", groupName)

	err := modifyGroupToAD(dnOfGroup, client, description, groupManager)
	if err != nil {
		log.Printf("[ERROR] Error while modifying a group: %s", err)
		return fmt.Errorf("Error while modifying a group: %s", err)
	}

	log.Printf("[DEBUG] Group modified successfully: %s", groupName)
	return nil
}
