...
func addGroupToAD(groupName string, dnName string, adConn *ldap.Conn, desc, gidNumber, groupManager string) error {
	...
	if groupManager != "" {
		addRequest.Attribute("managedBy", []string{groupManager})
	}
...
func modifyGroupToAD(dnName string, adConn *ldap.Conn, desc, groupManager string) error {
	req := ldap.NewModifyRequest(dnName, nil)
	if desc != "" {
		req.Replace("description", []string{desc})
	}
	if groupManager != "" {
		req.Replace("managedBy", []string{groupManager})
	}
	err := adConn.Modify(req)
	if err != nil {
		return err
	}
	return nil
