...
resource "sumologic_field_extraction_rule" "corelight-ssl" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,version,cipher,curve,server_name,resumed,last_alert,next_protocol,established,cert_chain_fuids,client_cert_chain_fuids,subject,issuer,client_subject,client_issuer,encrypted_dns_resp_h
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-weird" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,name,addl,notice
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-snmp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,duration,version,community,get_requests,get_bulk_requests,get_responses,set_requests,display_string,up_since
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-corelight_overall_capture_loss" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*" as ts,gaps,acks,percent_lost
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-radius" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,username,mac,framed_addr,tunnel_client,connect_info,reply_msg,result,ttl
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-smtp_links" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,fuid,link
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-kerberos" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,request_type,client,service,success,error_msg,from,till,cipher,forwardable,renewable,client_cert_subject,client_cert_fuid,server_cert_subject,server_cert_fuid
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-syslog" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,proto,facility,severity,message
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-notice" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,fuid,file_mime_type,file_desc,proto,note,msg,sub,src,dst,p,n,peer_descr,actions,suppress_for,remote_location_country_code,remote_location_region,remote_location_city,remote_location_latitude,remote_location_longitude
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-smtp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,trans_depth,helo,mailfrom,rcptto,date,from,to,cc,reply_to,msg_id,in_reply_to,subject,x_originating_ip,first_received,second_received,last_reply,path,user_agent,tls,fuids,is_webmail
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-rdp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,cookie,result,security_protocol,client_channels,keyboard_layout,client_build,client_name,client_dig_product_id,desktop_width,desktop_height,requested_color_depth,cert_type,cert_count,cert_permanent,encryption_level,encryption_method
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-ftp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,user,password,command,arg,mime_type,file_size,reply_code,reply_msg,data_channel_passive,data_channel_orig_h,data_channel_resp_h,data_channel_resp_p,fuid
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-ntp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,version,mode,stratum,poll,precision,root_delay,root_disp,ref_id,ref_time,org_time,rec_time,xmt_time,num_exts
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-pe" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,id,machine,compile_ts,os,subsystem,is_exe,is_64bit,uses_aslr,uses_dep,uses_code_integrity,uses_seh,has_import_table,has_export_table,has_cert_table,has_debug_data,section_names
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-dpd" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,proto,analyzer,failure_reason
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-tunnel" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,tunnel_type,action
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-dns" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,proto,trans_id,rtt,query,qclass,qclass_name,qtype,qtype_name,rcode,rcode_name,AA,TC,RD,RA,Z,answers,TTLs,rejected
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-sip" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uid,src_ip,src_port,dest_ip,dest_port,trans_depth,method,uri,date,request_from,request_to,response_from,response_to,reply_to,call_id,seq,subject,request_path,response_path,user_agent,status_code,status_msg,warning,request_body_len,response_body_len,content_type
      EOT
  enabled          = true
}
...
resource "sumologic_field_extraction_rule" "corelight-dhcp" {
  ...
  parse_expression = <<EOT
      parse "*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*" as ts,uids,client_addr,server_addr,mac,host_name,client_fqdn,domain,requested_addr,assigned_addr,lease_time,client_message,server_message,msg_types,duration
      EOT
  enabled          = true
}